#include "franka_dual_arm_joint_position_controller/dual_arm_position_controller.hpp"
...
DualArmPositionController::command_interface_configuration() const {
...
controller_interface::InterfaceConfiguration
DualArmPositionController::state_interface_configuration() const {
  return command_interface_configuration();         // 仅需回读 position
}

/* ---------------- lifecycle hooks ---------------- */
rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn
DualArmPositionController::on_init() {
  RCLCPP_INFO(get_node()->get_logger(), "DualArmPositionController on_init called!");
...
rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn
DualArmPositionController::on_activate(const rclcpp_lifecycle::State&) {
  // left_prefix_  = get_node()->get_parameter("left_prefix").as_string();
  // right_prefix_ = get_node()->get_parameter("right_prefix").as_string();
  // std::cerr<<"left_prefix_!!!!!!!!!!!!!!!!!! "<<left_prefix_<<std::endl;
  sub_ = get_node()->create_subscription<std_msgs::msg::Float64MultiArray>(
      "~/target_positions", 10,
      std::bind(&DualArmPositionController::topic_callback, this, std::placeholders::_1));

  srv_ = get_node()->create_service<franka_dual_msgs::srv::DualJointAngle>(
      "~/set_target",
      std::bind(&DualArmPositionController::service_callback,
                this, std::placeholders::_1, std::placeholders::_2));

  /* 初始化目标为当前位姿（防止跳变） */
  for (size_t i = 0; i < kNumJoints; ++i)
    target_[i] = state_interfaces_[i].get_value();
  last_valid_ = target_;
  return CallbackReturn::SUCCESS;
}
...
DualArmPositionController::update(const rclcpp::Time&, const rclcpp::Duration&) {
  for (size_t i = 0; i < kNumJoints; ++i)
...
/* ---------------- helper ---------------- */
bool DualArmPositionController::validate_size(const std::vector<double>& v) const {
...
/* -------- pluginlib export -------- */
PLUGINLIB_EXPORT_CLASS(franka_dual_arm_joint_position_controller::DualArmPositionController,
